 ...
 * 知乎"图片转移"面板。
 *
 * 为什么需要：知乎服务端拒收 data URL 形式的 <img>，HTML 粘贴会变成
 * "图片导入失败"占位。但知乎接受"剪贴板里直接装 image/png blob"——
 * 用户 Cmd+V 一次粘一张图，它会自动建图片块。所以这个面板的工作就是：
 * 列出原文里的所有图，每张配一个"复制"按钮，点完去知乎粘一下，循环。
 *
...
export default function ImageTransferPanel({
  ...
  async function handleCopy(idx: number) {
    ...
    if (ok) {
      ...
      onToast(`图 ${idx + 1} 已复制，去知乎 Cmd+V 粘贴`);
    } else {
  ...
  return (
    <div className="img-transfer-panel" role="dialog" aria-label="图片转移">
      <div className="img-transfer-head">
        <div className="img-transfer-title">
          知乎图片转移 · 共 {images.length} 张
        </div>
        <button
      ...
      <div className="img-transfer-hint">
        正文已复制 · 请先到知乎粘贴正文，再按顺序逐张点「复制」→ 在知乎 Cmd+V
      </div>
